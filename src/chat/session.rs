...
use crate::openai::model::{
    AssistantMessage, ChatMessage, ChatResult, OpenAiModel, Tool,
};
...
mod tests {
    use super::*;
    use crate::openai::model::{Function, Parameters, Property, ToolType};
    use std::collections::HashMap;

    #[test]
    fn new_creates_a_chat_session_with_empty_memory() {
        let openai_client = OpenAIClient::new("any_api_key".to_string());
        let chat_session = ChatSession::new(openai_client, vec![]);
    ...
    async fn send_message_adds_user_message_to_memory() {
        ...
        let mut chat_session = ChatSession::new(openai_client, vec![]);
