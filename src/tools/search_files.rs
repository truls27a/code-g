...
impl Tool for SearchFiles {
    ...
    fn approval_message(&self, args: &HashMap<String, String>) -> String {
        ...
        format!(
            "CodeG wants to search for files matching pattern {}",
            pattern
        )
    }
    ...
    fn call(&self, args: HashMap<String, String>) -> Result<String, String> {
        ...
        match Self::search_files(pattern, directory) {
            Ok((files, truncated)) => {
                if files.is_empty() {
...
impl SearchFiles {
    ...
    fn search_files(pattern: &str, directory: &str) -> Result<(Vec<String>, bool), String> {
        ...
        Self::search_directory_recursive(search_path, pattern, &mut found_files)?;
        found_files.sort();
    ...
    fn search_directory_recursive(
        ...
        for entry in entries {
            ...
            if path.is_dir() {
                ...
                Self::search_directory_recursive(&path, pattern, found_files)?;
    ...
    fn matches_pattern(filename: &str, pattern: &str) -> bool {
        ...
        Self::matches_with_wildcards(&filename_chars, &pattern_chars, 0, 0)
    }
    ...
    fn matches_with_wildcards(
        ...
        match pattern[p_idx] {
            '*' => {
                ...
                if Self::matches_with_wildcards(filename, pattern, f_idx, p_idx + 1) {
                    return true;
                }
                // Try matching one or more characters
                for i in f_idx..filename.len() {
                    if Self::matches_with_wildcards(filename, pattern, i + 1, p_idx + 1) {
                        return true;
                    }
                }
                false
            ...
            '?' => {
                ...
                Self::matches_with_wildcards(filename, pattern, f_idx + 1, p_idx + 1)
            }
            c => {
...
mod tests {
    ...
    fn matches_pattern_returns_true_for_matching_patterns() {
        ...
        assert!(SearchFiles::matches_pattern("main.rs", "*.rs"));
        assert!(SearchFiles::matches_pattern("test.rs", "*.rs"));
        assert!(!SearchFiles::matches_pattern("main.py", "*.rs"));
        assert!(SearchFiles::matches_pattern("main.rs", "main.rs"));
        assert!(!SearchFiles::matches_pattern("test.rs", "main.rs"));
        assert!(SearchFiles::matches_pattern("config.json", "config.*"));
        assert!(SearchFiles::matches_pattern("config.toml", "config.*"));
        assert!(!SearchFiles::matches_pattern("main.json", "config.*"));
        assert!(SearchFiles::matches_pattern("test_file.txt", "test_*"));
        assert!(SearchFiles::matches_pattern("any_file.txt", "*"));

        // Test ? wildcard patterns - matches exactly one character
        assert!(SearchFiles::matches_pattern("a.rs", "?.rs"));
        assert!(SearchFiles::matches_pattern("x.rs", "?.rs"));
        assert!(!SearchFiles::matches_pattern("ab.rs", "?.rs")); // ? doesn't match multiple chars
        assert!(!SearchFiles::matches_pattern(".rs", "?.rs")); // ? doesn't match zero chars

        assert!(SearchFiles::matches_pattern("test1.txt", "test?.txt"));
        assert!(SearchFiles::matches_pattern("testA.txt", "test?.txt"));
        assert!(!SearchFiles::matches_pattern("test12.txt", "test?.txt")); // ? doesn't match multiple chars
        assert!(!SearchFiles::matches_pattern("test.txt", "test?.txt")); // ? doesn't match zero chars

        // Test multiple ? wildcards
        assert!(SearchFiles::matches_pattern("abc.txt", "???.txt"));
        assert!(SearchFiles::matches_pattern("xyz.txt", "???.txt"));
        assert!(!SearchFiles::matches_pattern("ab.txt", "???.txt")); // not enough chars
        assert!(!SearchFiles::matches_pattern("abcd.txt", "???.txt")); // too many chars

        // Test combinations of ? and *
        assert!(SearchFiles::matches_pattern("a_test.rs", "?_*.rs"));
        assert!(SearchFiles::matches_pattern("x_anything.rs", "?_*.rs"));
        assert!(!SearchFiles::matches_pattern("_test.rs", "?_*.rs")); // ? must match one char
        assert!(!SearchFiles::matches_pattern("ab_test.rs", "?_*.rs")); // ? matches only one char
    }
